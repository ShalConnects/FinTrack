...
import { Account, Transaction, Category, Budget, DashboardStats, SavingsGoal, Purchase, PurchaseCategory, PurchaseAnalytics, MultiCurrencyPurchaseAnalytics, PurchaseAttachment, LendBorrowAnalytics } from '../types';
...
export const useFinanceStore = create<FinanceStore>((set, get) => ({
  ...
  addAccount: async (account: Omit<AccountInput, 'id' | 'user_id' | 'created_at'> & { dps_initial_balance?: number, transaction_id?: string }) => {
    ...
    try {
      ...
      if (account.has_dps) {
        console.log('Creating DPS savings account with:', {
        ...
        console.log('DPS savings account creation result:', { data: savingsAccount, error: savingsError });
  ...
  updateAccount: async (id, updates) => {
    try {
      ...
      dbUpdates.dps_fixed_amount = updates.has_dps && updates.dps_amount_type === 'fixed' ? updates.dps_fixed_amount : null;
      ...
      if (updates.has_dps && !currentAccount.has_dps) {
        ...
        if (currentAccount.dps_savings_account_id) {
          ...
          const { error: dpsUpdateError } = await supabase
            .from('accounts')
            .update({ initial_balance: dps_initial_balance, updated_at: new Date().toISOString() })
            .eq('id', currentAccount.dps_savings_account_id);
          console.log('DPS savings account initial_balance updated:', { dps_savings_account_id: currentAccount.dps_savings_account_id, dps_initial_balance, dpsUpdateError });
          ...
          console.log('DPS savings account creation for existing account result:', { data: savingsAccount, error: savingsError });
          if (savingsError) throw savingsError;
  ...
  addTransaction: async (transaction: Omit<Transaction, 'id' | 'created_at'> & { transaction_id?: string }, purchaseDetails?: {
    ...
    // If this is an expense transaction with a purchase category, create a purchase record
    if (transactionData.type === 'expense' && data?.id) {
  ...
  getDashboardStats: () => {
    ...
    const byCurrency = activeAccounts.reduce((acc: any[], account) => {
      ...
      if (!currencyGroup) {
        ...
        // Debug logging
        console.log(`Dashboard stats for ${account.currency}:`, {
          currency: account.currency,
  ...
  setShowAccountForm: (show: boolean) => {
    console.log('setShowAccountForm called with:', show);
    set({ showAccountForm: show });
  },
  ...
  syncExpenseCategoriesWithPurchaseCategories: async () => {
    ...
    // Get all expense categories (including default ones from memory)
    const allCategories = [...defaultCategories, ...get().categories];
    const expenseCategories = allCategories.filter(cat => cat.type === 'expense');
    const existingPurchaseCategories = get().purchaseCategories;
    
    console.log('Syncing expense categories with purchase categories:', {
      allCategories: allCategories.map(c => ({ name: c.name, type: c.type })),
      expenseCategories: expenseCategories.map(ec => ec.name),
      existingPurchaseCategories: existingPurchaseCategories.map(pc => pc.category_name)
    });
    
    for (const expenseCat of expenseCategories) {
      const hasPurchaseCategory = existingPurchaseCategories.some(
        pc => pc.category_name === expenseCat.name
      );
      
      console.log(`Checking category "${expenseCat.name}":`, { hasPurchaseCategory });
      
      if (!hasPurchaseCategory) {
        console.log(`Creating missing purchase category for "${expenseCat.name}"`);
        ...
        if (error) {
          console.error('Error syncing purchase category:', error);
        } else if (savedCategory) {
          console.log(`Successfully created purchase category:`, savedCategory);
          // Update local state
          set((state) => ({ 
