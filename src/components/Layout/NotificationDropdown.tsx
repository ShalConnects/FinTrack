...
export function NotificationDropdown({ iconClassName = "w-5 h-5 text-gray-500 dark:text-gray-300" }: { iconClassName?: string }) {
  const {
    notifications,
    unreadCount,
    isLoading,
  ...
  return (
    <Menu as="div" className="relative">
      <Menu.Button className="relative flex items-center justify-center p-0" style={{ height: 30, width: 30 }}>
        ...
        {unreadCount > 0 && (
          <span
            ...
            style={{ top: '-0.15rem', right: '-0.15rem', height: '0.85rem', width: '0.85rem', fontSize: '11px' }}
          >
            {unreadCount}
          </span>
        )}
                        ...
                        <div className="flex-1">
                          ...
                          {notification.body && (
                            <p className="mt-1 text-sm text-gray-500">{
                              // Strip [ID:...] marker from the end of the body
                              notification.body.replace(/\[ID:[^\]]+\]$/, '').trim()
                            }</p>
                          )}
