...
export const GlobalSearchDropdown: React.FC<GlobalSearchDropdownProps> = ({ isFocused, inputRef, dropdownRef, onClose }) => {
  const { globalSearchTerm, transactions, accounts, setGlobalSearchTerm, purchases } = useFinanceStore();
  ...
  const handleResultClick = (type: string, item: any) => {
    ...
    switch (type) {
      case 'account':
        ...
        navigate(`/dashboard/accounts?selected=${itemId}`);
        break;
      case 'transaction':
        console.log('Navigating to transaction:', itemId);
        navigate(`/dashboard/transactions?selected=${itemId}`);
        break;
      case 'purchase':
        console.log('Navigating to purchase:', itemId);
        navigate(`/dashboard/purchases?selected=${itemId}`);
        break;
      case 'transfer':
        console.log('Navigating to transfer:', itemId);
        navigate(`/dashboard/transfers?selected=${itemId}`);
        break;
      default:
  ...
  // Debug logging
  console.log('GlobalSearchDropdown:', {
    globalSearchTerm,
    transactionsLength: transactions.length,
    accountsLength: accounts.length,
    fuzzyAccounts: fuzzyAccounts.length,
    search,
  });

  // Calculate total results for keyboard navigation
  const totalResults = fuzzyTransactions.length + fuzzyPurchases.length + fuzzyTransfers.length + fuzzyAccounts.length;
    ...
    const handler = (e: KeyboardEvent) => {
      ...
      if (e.key === 'Enter') {
        ...
        if (search) {
          ...
          } else if (fuzzyAccounts.length > 0 && highlightedIdx >= fuzzyTransactions.length + fuzzyPurchases.length + fuzzyTransfers.length) {
  ...
  }, [isFocused, search, highlightedIdx, fuzzyTransactions, fuzzyPurchases, fuzzyTransfers, fuzzyAccounts, recentSearches, setGlobalSearchTerm]);
            ...
            {recentSearches.slice(0, 3).map((term, idx) => (
              <li
                ...
                onClick={(e) => {
                  ...
                  e.preventDefault();
                  e.stopPropagation();
                  console.log('Setting global search term to:', term);
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* No Results */}
        {search && fuzzyTransactions.length === 0 && fuzzyPurchases.length === 0 && fuzzyTransfers.length === 0 && fuzzyAccounts.length === 0 && (
