...
export const TransactionsView: React.FC = () => {
  const { transactions, accounts, categories, loading, error, globalSearchTerm, fetchTransactions } = useFinanceStore();
  const [searchParams, setSearchParams] = useSearchParams();
  const [showTransactionForm, setShowTransactionForm] = useState(false);
  const prevShowTransactionForm = useRef(showTransactionForm);
  ...
  useEffect(() => {
    // When modal closes (showTransactionForm goes from true to false), refresh transactions
    if (prevShowTransactionForm.current && !showTransactionForm) {
      fetchTransactions && fetchTransactions();
    }
    prevShowTransactionForm.current = showTransactionForm;
  }, [showTransactionForm, fetchTransactions]);
