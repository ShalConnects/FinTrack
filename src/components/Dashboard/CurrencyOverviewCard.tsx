...
export const CurrencyOverviewCard: React.FC<CurrencyOverviewCardProps> = ({
  ...
  // Debug logging for currency card
  console.log(`CurrencyOverviewCard Debug for ${currency}:`, {
    currency,
    ...
    incomeTransactions: filteredTransactions.filter(t => t.type === 'income').map(t => ({
      date: t.date,
      amount: t.amount,
      description: t.description,
      transactionDate: new Date(t.date).toISOString()
    })),
    allTransactionsInPeriod: filteredTransactions.map(t => ({
      date: t.date,
      amount: t.amount,
      type: t.type,
      description: t.description,
    ...
    <div className="bg-white dark:bg-gray-800 rounded-xl p-4">
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2 flex-1">
          <h2 className="text-lg font-bold text-gray-900 dark:text-white">{t('dashboard.currencyOverview', { currencyName: t(`currencies.${currency}`, currency) })}</h2>
